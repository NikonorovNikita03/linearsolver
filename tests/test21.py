...
class TransportationSolver(QMainWindow):
    ...
    def process_text_input(self):
        try:
            ...
            _, _, supply, demand, _, costs = tpp.parse_transport_problem()[0]
            
            # Обновляем размеры таблиц
            self.source_spin.setValue(len(supply))
            self.dest_spin.setValue(len(demand))
            self.update_table_size()
            
            # Заполняем таблицу стоимостей
            for i in range(len(costs)):
                for j in range(len(costs[0])):
                    item = QTableWidgetItem(str(costs[i][j]))
                    item.setTextAlignment(Qt.AlignCenter)
                    self.costs_table.setItem(i, j, item)
            
            # Заполняем таблицу поставщиков
            for i in range(len(supply)):
                item = QTableWidgetItem(str(supply[i]))
                item.setTextAlignment(Qt.AlignCenter)
                self.supply_table.setItem(i, 0, item)
            
            # Заполняем таблицу потребителей
            for j in range(len(demand)):
                item = QTableWidgetItem(str(demand[j]))
                item.setTextAlignment(Qt.AlignCenter)
            ...
            self.show_status_message("Данные успешно загружены из текста!")
        
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Не удалось обработать введенные данные:\n{str(e)}")
