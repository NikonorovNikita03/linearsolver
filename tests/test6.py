...
from PySide6.QtWidgets import (
    ...
    QLabel, QSpinBox, QComboBox, QTextEdit, QMessageBox
)
...
class TransportationSolver(QMainWindow):
    ...
    def create_input_table(self):
        ...
        # Copy button
        copy_btn = QPushButton("Copy Table Data")
    ...
    def create_output_display(self):
        ...
        solution_copy_btn = QPushButton("Copy Solution Data")
    ...
    def copy_table_data(self, table):
        ...
        self.statusBar().showMessage("Table data copied to clipboard!", 3000)
    ...
    def paste_data_to_table(self):
        ...
        if not rows:
            self.statusBar().showMessage("No valid data in clipboard!", 3000)
            return
        ...
        if len(data) != expected_rows or any(len(row) != expected_cols for row in data):
            QMessageBox.warning(
                self,
                "Size Mismatch",
                f"Clipboard data size ({len(data)}x{len(data[0]) if data else 0}) "
                f"doesn't match table size ({expected_rows}x{expected_cols})",
                QMessageBox.Ok
            )
            return
        ...
        try:
            ...
            self.statusBar().showMessage("Data pasted successfully!", 3000)
        except Exception as e:
            QMessageBox.critical(
                self,
                "Paste Error",
                f"Failed to paste data: {str(e)}",
                QMessageBox.Ok
            )
...
"""
        ...
        self.statusBar().showMessage("Problem solved!", 3000)
