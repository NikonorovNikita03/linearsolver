...
class TransportationSolver(QMainWindow):
    ...
    def create_input_table(self):
        ...
        # Enable copying
        self.input_table.setSelectionMode(QTableWidget.ContiguousSelection)
        self.input_table.setContextMenuPolicy(Qt.CustomContextMenu)
        self.input_table.customContextMenuRequested.connect(lambda pos: self.show_context_menu(pos, self.input_table))
    ...
    def create_output_display(self):
        ...
        # Enable copying for solution table
        self.solution_table.setSelectionMode(QTableWidget.ContiguousSelection)
        self.solution_table.setContextMenuPolicy(Qt.CustomContextMenu)
        self.solution_table.customContextMenuRequested.connect(lambda pos: self.show_context_menu(pos, self.solution_table))
    ...
    def keyPressEvent(self, event):
        """Handle Ctrl+C for copying table data from focused table"""
        if event.key() == Qt.Key_C and event.modifiers() == Qt.ControlModifier:
            focused_widget = QApplication.focusWidget()
            if isinstance(focused_widget, QTableWidget):
                self.copy_table_to_clipboard(focused_widget)
        else:
            super().keyPressEvent(event)
    
    def show_context_menu(self, position, table):
        """Show context menu with copy option"""
        menu = table.createStandardContextMenu()
        
        # Add custom copy action
        copy_action = menu.addAction("Copy (Ctrl+C)")
        copy_action.triggered.connect(lambda: self.copy_table_to_clipboard(table))
    ...
    def copy_table_to_clipboard(self, table):
        ...
        # Include headers if the entire column/row is selected
        include_headers = False
        if selection.leftColumn() == 0 and selection.rightColumn() == table.columnCount() - 1:
            include_headers = True
        
        # Build the text with tabs and newlines
        data = []
        
        # Add column headers if needed
        if include_headers:
            header_row = []
            if selection.topRow() == 0:  # Only if top row is selected
                header_row.append("")  # Empty cell for row header column
                for col in cols:
                    header_row.append(table.horizontalHeaderItem(col).text() if col < table.columnCount() else "")
        ...
        for row in rows:
            ...
            if include_headers and selection.leftColumn() == 0:
                row_data.append(table.verticalHeaderItem(row).text() if row < table.rowCount() else "")
